...
function Grid(hiddenField) {
    ...
    /** 0=處理者,1=建立時間,2=簽核時間,3=簽核意見,4=關卡名稱,5=簽核狀態 */
    grid.asExecutiveCommentHistory = function (displayIds) {
        if (serialNumber) {
            DWREngine.setAsync(false);
            ajax_ProcessAccessor.findExecutiveCommentBySN(serialNumber, function (executiveComments) {
                if (executiveComments.length > 0) {
                    var gridRowDatas = [];

                    executiveComments.forEach(function (executiveComment, index, array) {
                        var gridRowData = [];

                        displayIds.forEach(function (displayId, index, array) {
                            var rowData = executiveComment[displayId];
                            if (rowData) {
                        ...
                        gridRowDatas.push(gridRowData);
                    });

                    grid.clear();
                    grid.display(gridRowDatas);
                }
            });
        }
    }
...
function TextBox(textBox) {
    ...
    /** 隱藏系統簽核意見文字框,用自訂文字框取代 */
    textBox.asExecutiveComment = function () {
        var txaExecutiveComment = window.parent.document.getElementById("txaExecutiveComment");
        if (txaExecutiveComment) {
            var syncValueFunc = "(function(){ txaExecutiveComment.value = document.getElementById('ifmAppLocation').contentDocument.getElementById('{0}').value; })();".format(textBox.id);
            var btnDispatchWorkItem = window.parent.document.getElementById("btnDispatchWorkItem");
            if (btnDispatchWorkItem) btnDispatchWorkItem.setAttribute("onclick", syncValueFunc + btnDispatchWorkItem.getAttribute("onclick"));
            var btnReexecuteActivity = window.parent.document.getElementById("btnReexecuteActivity");
            if (btnReexecuteActivity) btnReexecuteActivity.setAttribute("onclick", syncValueFunc + btnReexecuteActivity.getAttribute("onclick"));
            var btnTerminateProcess = window.parent.document.getElementById("btnTerminateProcess");
            if (btnTerminateProcess) btnTerminateProcess.setAttribute("onclick", syncValueFunc + btnTerminateProcess.getAttribute("onclick"));
            var btnReassignActivity = window.parent.document.getElementById("btnReassignActivity");
            if (btnReassignActivity) btnReassignActivity.setAttribute("onclick", syncValueFunc + btnReassignActivity.getAttribute("onclick"));
        }
    }
